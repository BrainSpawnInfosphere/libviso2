...
vector<double> VisualOdometryMono::estimateMotion (vector<Matcher::p_match> p_matched) {
  ...
  Matrix X,R,t;
  EtoRt(E,K,p_matched,inliers,X,R,t);
...
void VisualOdometryMono::EtoRt(Matrix &E,Matrix &K,vector<Matcher::p_match> &p_matched,vector<int32_t> &inliers,Matrix &X,Matrix &R,Matrix &t) {
